...
function verifieDate(leChamps,message,minimum){
	//Au viveau de la saisie, on n'a verifie uniquement qu'il n'y avait que des valeurs num?riques et des /
	  // on vient de v?rifier la longueur.
	  // Verif N?1 : regex
...
function afficheChampsenSus(leFormulaire,leChamps) {
		...
		//Parcours de tous les champs input qui ne sont pas cach?s"
		//Si dans la liste de ceux ? conserver ou ajouter, on vire hide sur le parent qui va bien sinon on le met
		//
...
function afficheForm(nomForm) {
	...
	switch(nomForm) {
		case 'CTX' :
								...
								/*$('#btnAction').html('Continuer').addClass('btn-default').removeClass('btn-primary btn-glow').addClass('disabled').parent().attr({ 'data-original-title': 'Pour continuer, veuillez saisir votre num%E9ro fiscal.', 'data-toggle': 'tooltip', 'data-placement': 'bottom' });*/
...
//								NoError();
		...
		case '3S' :
								...
								// On masque les rubriques d'aide qui seraient d?j? d?pli?es
								$('details').removeAttr("open");
								// Et on d?plie les rubriques d'aide en fonction du focus
			...
			case 'PAS' :
								...
								// On d?plie par d?faut la rubrique d'aide relative ? la perte du mot de passe
		...
		switch (nomFormulaire)
			{
						...
						NoError();
							...
							else if(!exactement('teledec',7,'Votre num?ro d\'acc?s en ligne comporte 7 chiffres&nbsp;;&nbsp;veuillez v?rifier votre saisie.')) { nbErr++;  if(focusTo.length==0)focusTo="teledec";  }
						...
						NoError();
							...
							if(nbCat2) $('#erreur').html('Vous devez saisir votre num?ro fiscal ? 13 chiffres.').removeClass('alert alert-info hide').addClass('alert alert-danger text-center');
							if(nbErr) { if(focusTo.length!=0)donneFocus(nomFormulaire,focusTo) ;  return(false); }
						...
						NoError();
							break;
						// ---------------------------------------------
						// L'acc?s par l'utilisation du compte d?j? cr??
						case 'LMDP':
						NoError();
							...
							break;
						// ------------------------------------------------------------------------------------
						// La cr?ation du compte par la saisie de la date de naissance si le contexte s'y pr?te
						case 'PAS':
							NoError();
							if (!obligatoire('spi') ) { nbCat2++; nbErr++; if(focusTo.length==0)focusTo="spi";  }
							else if(!exactement('spi',13, messageContenu['badFormatSpi'])) { nbErr++; }
						...
						NoError();
							...
							//else if(!exactement('numfiscal',13,'Votre num?ro fiscal comporte 13 chiffres&nbsp;;&nbsp;veuillez v?rifier votre saisie.')) { nbErr++; }
...
function ecouteReponseForm()
	{
		...
		eventer(messageEvent,function(e) {
			...
			switch(message) {
				case 'ctx': {
					...
					switch(value) {
						...
						case 'MPA_LINK':
							...
							if(authType=="idp") {
								...
								$('#FranceConnect').removeClass('hide');
								afficheForm(value.replace("_LINK",""));
							}
					...
					case 'ERR' :
						...
						afficheForm('CTX');
						$('#FranceConnect').removeClass('hide');
						$('#erreur').html('L\'application est indisponible ; veuillez r?essayer ult?rieurement.').removeClass('hide alert-info').addClass('alert alert-danger');
						break;
					case 'NOINFO':
						NoError();
						$('#FranceConnect').removeClass('hide');
			...
			//------------------------------------------------------------------------------------------------------------------
			case 'ok':
				//$('#erreur').html('Authentification r?ussie.').removeClass('hide alert-danger text-center').addClass('alert alert-info');
...
//--------------------------------------------------------------------------------------------------------------------------------------
//Controle des champs de saisie a chaque modification - s'appuie sur  balises libres data-* cot? html pour le parametrage des traitements
//--------------------------------------------------------------------------------------------------------------------------------------
		...
		if (mask) {
			...
			if(propre.length<lgMax) {
				if (feedbackOK) {
					$(this).css({ color: '#555' });
					if($('#formulairePrincipal').attr('name')=='CTX') {
						/*$('#btnAction').attr('aria-label','Pour continuer, veuillez saisir votre num%E9ro fiscal.').addClass('btn-default').removeClass('btn-primary btn-glow').addClass('disabled').parent().attr({ 'data-original-title': 'Pour continuer, veuillez saisir votre num%E9ro fiscal.', 'data-toggle': 'tooltip', 'data-placement': 'bottom' })*/
					}
				}
			}
			oldInput=propre;
...
function traiteOubli(oubli,retour,indTel,numTel){
	...
	switch(retour) {
		...
		case (retour.match(/^RENEW_LAST_[0-9]{1,2}/) || {}).input : 
			...
			$('#'+nomFormSMS).find('button').text(messageContenu['boutonSMS'+nomModOubli]);
			//$('#infoLastSMS').html(messageContenu['SMSRENEWLAST'].replace("__N__",messageAttente)).removeClass('hide text-center alert-info alert-success').addClass('alert alert-error');
				//$("#erreurModSMS").html(message+" (apres la fin du d?lai indiqu?)").removeClass('hide text-center').addClass('alert alert-danger');
         decompte('infoLastSMS',attente);
			$("#msgInfoSMS").removeClass('alert alert-danger hide text-center').addClass('alert alert-info');
			$("#"+"P_codeNonRecu").addClass('hide');
			if (ITX==1) $('#msgInfoSMSITX').html(messageContenu['SMSITX'].replace('__CODE__',ss)).removeClass('hide text-center alert-info alert-error').addClass('alert alert-success');
...
function traite3S(retour)
	{
		...
		switch(retour) {
			...
			case '5001':
			case '5009':
				$('#erreur').html('Les donn?es saisies sont incorrectes.<br>V?rifier la saisie de vos identifiants.').removeClass('hide alert-info').addClass('alert alert-danger');
			...
			//---------------------------------------------------------------------
			case '5024':
				$('#erreur').html('Format des donn?es incorrect.<br>V?rifier la saisie de vos identifiants.').removeClass('hide alert-info').addClass('alert alert-danger');
				break;
		}
	}
//--------------------------------------------------------------------------------
...
function traitePAS(retour)
	{
		...
		switch(code) {
			...
			case '7005':
				$('#blocage').html('').removeClass('alert alert-danger').addClass('alert alert-info hide ');
				$('#erreur').html('Les donn?es saisies sont incorrectes.<br>Veuillez v?rifier la saisie de votre date de naissance.').removeClass('hide alert-info text-center').addClass('alert alert-danger');
			...
			case '7024':
				...
				break;
			case '7020':
				$('#erreur').html('? la suite de plusieurs saisies erron?es, votre acc?s est suspendu pendant '+ message ).removeClass('hide alert-info').addClass('alert alert-danger');
				$('#blocage').html('').removeClass('alert-info').addClass('alert alert-danger hide');
				break;
			default :
				afficheForm('CTX');	
...
// Incr�menter le compteur de pages depuis FC
